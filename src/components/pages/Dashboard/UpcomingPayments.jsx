...
const UpcomingPayments = () => {
  const { upcomingPayment, accountBalance, loading, formatDate } = useDashboardData();
  ...
  const billingPeriod = upcomingPayment.nextPeriodStart && upcomingPayment.nextPeriodEnd
    ? `${formatDate(upcomingPayment.nextPeriodStart)} — ${formatDate(upcomingPayment.nextPeriodEnd)}`
    : 'N/A';

  const nextPaymentDate = upcomingPayment.date
    ...
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      <div className="divide-y">
          ...
          <div className="flex items-start gap-3 md:gap-4 p-4 md:p-5">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs md:text-sm text-gray-700 break-words">
                Upcoming Payment: <span className="font-semibold">{currencyKES(upcomingPayment.amount)}</span> will be auto‑processed from your linked account on <span className="font-medium">{nextPaymentDate}</span>
              </div>
        ...
        <div className="flex items-start gap-3 md:gap-4 p-4 md:p-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="text-xs md:text-sm text-gray-700 truncate">Account Balance: <span className="font-semibold">{currencyKES(accountBalance)}</span></div>
