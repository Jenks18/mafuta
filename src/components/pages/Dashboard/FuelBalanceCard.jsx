...
const FuelBalanceCard = () => {
  const { fuelCardBalance, primaryFuelCard, loading } = useDashboardData();
  ...
  // Extract card details from database or use defaults
  const cardNumber = primaryFuelCard?.card_number || '';
  const last4 = cardNumber.slice(-4) || '****';
  const cardHolder = primaryFuelCard?.card_holder_name || 'Fleet Card';
  const provider = primaryFuelCard?.provider || 'Shell Kenya';
